...
const iconMap: Record<string, any> = { Cpu, Shield, Zap, Wifi, Database, Cloud };

const defaultData = {
  subtitle: "Technology",
  title_prefix: "Our",
  title_highlight: "Tech Stack",
  description: "Pioneering hardware innovation with sustainability at every layer of our technology.",
  items: [
...
const TechStackSection = () => {
  const { data: content } = useSiteContent("techstack");
  const d = content || defaultData;
  const items = d.items || defaultData.items;

  return (
    <section id="tech" className="py-20 px-6 bg-secondary/40">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-bold mt-4" style={{ fontFamily: "var(--font-heading)" }}>
            {d.title_prefix || defaultData.title_prefix} <span className="text-gradient">{d.title_highlight || defaultData.title_highlight}</span>
          </h2>
          <p className="text-muted-foreground mt-4 max-w-2xl mx-auto text-lg">
            {d.description || defaultData.description}
          </p>
        </motion.div>
